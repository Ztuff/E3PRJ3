...
namespace osapi
{
  ...
  void Thread::start()
  {
    ...
    if(getuid() == 0) // Check to see if we are root
    {
      ...
      if(pthread_create(&threadId_, &attr, tf_->threadMapper, tf_) != 0) 
      	throw ThreadError();
      pthread_attr_destroy(&attr);
    }
    else
    {
      if(pthread_create(&threadId_, NULL, tf_->threadMapper, tf_) != 0) 
      	throw ThreadError();
    }
  ...
  void Thread::setPriority(Thread::ThreadPriority priority) 
  {
    priority_ = priority;
  	...
  	int policy;
  	struct sched_param* param = NULL;
    if(pthread_getschedparam(threadId_, &policy, param)) throw ThreadError();
    param->sched_priority = static_cast<int>(priority);
    if(pthread_setschedparam(threadId_, policy, param)) throw ThreadError();
  }
